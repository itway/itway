...
@else
  @include('includes.tag-side', [$tags, $tags_title = trans("team.team-tags")])
                    ...
                    <div class="l-12 m-12 s-12 xs-12">
                        <div class="slot">
                            <div class="slot-info-block l-6  m-6  s-6 xs-6">
                                ...
                                <img class="logo" src="{!! asset('images/teams/' . $team->logo_bg) !!}" alt="{{ $team->name }}"/>
                                <div class="name">
                            ...
                            <nav class="button-nav-slot button-group-horizontal  l-12 m-12 s-12 xs-12">
                                <span class="button">
                    <span class="text-left text-primary">{{$team->views_count()}}</span><i class="icon-remove_red_eye"></i></span>
                                <span class="button">
                                    <a class="text-left text-primary" href="{{ url(App::getLocale().'/team/'.$team->id.'#disqus_thread') }}" data-disqus-identifier="{{$team->id}}">0</a>
                                    <i  class="icon-comment"></i>
                                </span>
                    </nav>
                    <a class="read-slot button button-dark" href="{{url(App::getLocale().'/team/'.$team->id)}}">jump to team</a>
                    <span class="slot-time pull-left"><i class="icon-access_time text-warning"></i>{{$team->created_at->diffForHumans()}}</span>
                </div>
    </div>
  @endforeach
                </div>
