...
use TagsCloud\Tagging\Model\EventTagged;
use TagsCloud\Tagging\Model\PostTagged;
use TagsCloud\Tagging\Model\JobHuntTagged;
use TagsCloud\Tagging\Model\OpenSourceIdeaTagged;
...
trait Taggable
{
	 ...
	 * @return EventTagged|JobHuntTagged|OpenSourceIdeaTagged|PostTagged|TeamTagged|UserTagged
	 */
	public function getTaggedInstance(array $data = null){
		...
		if (strtolower($this->getShortModelName()) == "team"){
			return empty($data)? new TeamTagged() : new TeamTagged($data);
		}

		if (strtolower($this->getShortModelName()) == "teamworkteam"){

			return empty($data)? new TeamTagged() : new TeamTagged($data);
		}

		if (strtolower($this->getShortModelName()) == "event"){

			return empty($data)? new EventTagged() : new EventTagged($data);
		}
		...
		if (strtolower($this->getShortModelName()) == "jobhunt"){

			return empty($data)? new JobHuntTagged() : new JobHuntTagged($data);
		}

		if (strtolower($this->getShortModelName()) == "opensourceidea"){

			return empty($data)? new OpenSourceIdeaTagged() : new OpenSourceIdeaTagged($data);
		}
	}
